...
let getPrint exp (cmd: AbstractEngine<'Text, 'LabelName, 'Addon, 'Arg>) =
    match cmd with
...
let getMenu exp (cmd: AbstractEngine<'Text, 'LabelName, 'Addon, 'Arg>) =
    match cmd with
...
let getEnd (cmd: AbstractEngine<'Text, 'LabelName, 'Addon, 'Arg>) =
    match cmd with
    | AbstractEngine.End -> ()
    | x ->
        failwithf "expected End but:\n%A" x

let testCustomStatement (exp: 'Addon) eq (cmd: AbstractEngine<'Text, 'LabelName, 'Addon, 'Arg>) =
    match cmd with
